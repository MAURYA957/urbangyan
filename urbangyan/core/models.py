...
class Subjectcatogary(models.Model):
...
class Subject(models.Model):
    name = models.CharField(max_length=255)
    Subjectcatogary = models.ForeignKey(Subjectcatogary, on_delete=models.CASCADE, related_name="subject", default="others" )
    image = models.ImageField(
        upload_to="subjects/", blank=True, null=True
    )  # Optional image for Subject
    authors = models.TextField(
        blank=True, null=True, help_text="Enter author names separated by commas"
    )  # Store multiple author names
    is_active = models.BooleanField(default=True)
...
class Topic(models.Model):
    ...
    subject = models.ForeignKey(
        Subject, on_delete=models.CASCADE, related_name="topics", blank=True, null=True
    )
    unit = models.ForeignKey(
        Unit, on_delete=models.CASCADE, related_name="topics", blank=True, null=True
    )
    ...
    )  # Optional image for Topic
    file = models.FileField(
        upload_to="topics/files/", blank=True, null=True
    )  # File like PDF
...
class Questions(models.Model):
    ...
    question_level = models.CharField(
        max_length=50, default="Beginner"
    )  # Example default level
    unit = RichTextUploadingField(
        max_length=1000, null=True, blank=True, default="others"
    )  # for answer explanation added on 21/10/2024
    topic = RichTextUploadingField(
        max_length=1000, null=True, blank=True, default="others"
    )  # for answer explanation added on 21/10/2024
    question = RichTextUploadingField()
    option_1 = RichTextUploadingField()
    option_2 = RichTextUploadingField()
    option_3 = RichTextUploadingField()
    option_4 = RichTextUploadingField()
...
class UserSession(models.Model):
    ...
    )  # Set blank=True if you want it to be optional
...
class MockTestSubjectConfig(models.Model):
    mock_test = models.ForeignKey(
        "MockTest", on_delete=models.CASCADE, related_name="subject_configs"
    )
    subject = models.ForeignKey(
        "Subject", on_delete=models.CASCADE, related_name="mock_test_subject_configs"
    )
    num_questions = (
    ...
    max_score = (
        models.PositiveIntegerField()
    )  # Maximum score for this subject in the test
...
class MockTest(models.Model):
    ...
    def populate_questions(self):
        ...
        self.total_questions = (
            configs.aggregate(total_q=Sum("num_questions"))["total_q"] or 0
        )
        self.total_max_score = (
            configs.aggregate(total_score=Sum("max_score"))["total_score"] or 0
        )
        ...
        for config in configs:
            ...
            sampled_questions = random.sample(
                subject_questions, min(len(subject_questions), config.num_questions)
            )
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new_instance:
            ...
            super().save(
                update_fields=["total_questions", "total_max_score"]
            )  # Save totals without recursion
...
class UserResponse(models.Model):
    ...
    answer_description = RichTextUploadingField(
        max_length=1000, null=True, blank=True
    )  # Explanation based on user input
    is_correct = models.BooleanField(default=False)
    exam_name = models.CharField(
        max_length=255, null=True, blank=True
    )  # Changed from RichText to CharField
    ...
    def save(self, *args, **kwargs):
        logger.debug(
            f"Starting save process for UserResponse ID: {self.id if self.id else 'New Instance'}"
        )
        ...
        if self.selected_option not in [0, 1, 2, 3, 4]:
            logger.error(
                f"Invalid selected option: {self.selected_option}. Must be between 1 and 4."
            )
    ...
    def _populate_answer_description(self):
        ...
        self.answer_description = options_map.get(
            self.selected_option, "Invalid selection."
        )
...
class Advertisement(models.Model):
    ...
    google_ad_code = models.TextField(
        blank=True, null=True, help_text="Google Ad code (if applicable)."
    )
    url = models.URLField(
        blank=True, null=True, help_text="URL the advertisement links to."
    )
...
class JobType(models.Model):
    name = models.CharField(
        max_length=50, unique=True, help_text="Type of job (e.g., Sarkari, Private)."
    )
...
class JobCategory(models.Model):
    name = models.CharField(
        max_length=100,
        unique=True,
        help_text="Category of the job (e.g., Engineering, Medical).",
    )
...
class Job(models.Model):
    job_type = models.ForeignKey(
        JobType, on_delete=models.PROTECT, help_text="Type of job (Sarkari or Private)."
    )
    job_category = models.ForeignKey(
        JobCategory, on_delete=models.PROTECT, help_text="Category of the job."
    )
    ExperienceLevel = models.ForeignKey(
        ExperienceLevel,
    ...
    total_post = models.PositiveIntegerField(
        help_text="Total number of posts available."
    )
    eligibility = RichTextUploadingField(help_text="Eligibility criteria for the job.")
    important_date = RichTextUploadingField(
        help_text="Important dates related to the job."
    )
    stage = models.ForeignKey(
        JobStage, on_delete=models.PROTECT, help_text="Current stage of the job."
    )
    notification = models.FileField(
        upload_to="notification/",
        blank=True,
        null=True,
        help_text="Notification details.",
    )
    details = RichTextUploadingField(help_text="Detailed description of the job.")
    apply_link = models.URLField(
        blank=True, null=True, help_text="Link to apply for the job."
    )
    admit_card_link = models.URLField(
        blank=True, null=True, help_text="Link to download the admit card."
    )
    result_link = models.URLField(
        blank=True, null=True, help_text="Link to check the result."
    )
    official_website = models.URLField(help_text="Official website for the job.")

    created_at = models.DateTimeField(
        auto_now_add=True, help_text="The date and time when the job was created."
    )
    updated_at = models.DateTimeField(
        auto_now=True, help_text="The date and time when the job was last updated."
    )
...
class SavedJob(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, help_text="The user who saved the job."
    )
    ...
    created_at = models.DateTimeField(
        auto_now_add=True, help_text="The date and time when the job was saved."
    )
...
class Order(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.order_id:
            ...
            max_order_number = (
                Order.objects.filter(created_at__date=today).aggregate(Max("id"))[
                    "id__max"
                ]
                or 0
            )
...
class AffairsCategory(models.Model):
    ...
    description = models.TextField(
        blank=True, null=True, help_text="Optional description for the category"
    )
    created_at = models.DateTimeField(
...
class CurrentAffair(models.Model):
    ...
    description = models.TextField(
        help_text="Detailed description of the current affair"
    )
    ...
    country = models.CharField(
        max_length=100, help_text="Country related to the current affair"
    )
    source = models.URLField(
        blank=True, null=True, help_text="Source URL for the current affair"
    )
    created_at = models.DateTimeField(
...
class AdSenseConfig(models.Model):
    ...
    description = models.CharField(
        max_length=100, blank=True, help_text="Optional description for this ad slot"
    )
...
class News(models.Model):
    ...
    language = models.CharField(max_length=10, default="hi")  # Example: 'hi', 'en'
    state = models.CharField(max_length=100, null=True, blank=True)  # If applicable
    country = models.CharField(max_length=50, default="IN")  # Example: 'IN', 'US'
    source = models.CharField(max_length=255, null=True, blank=True)
    category = models.CharField(max_length=255, null=True, blank=True)  # Corrected case
